...
export default function AdminDashboard() {
  ...
  return (
    <div>
      ...
      {/* Stats — client component handles hover */}
      <AdminDashboardCards stats={stats} />

      {/* Recent reservations */}
      <div style={{ background: '#1a1d27', border: '1px solid #2d3147', borderRadius: '12px', overflow: 'hidden', marginBottom: '1.5rem' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              ...
              {recentReservations.length === 0 && (
                <tr><td colSpan={5} style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>No reservations yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '0.75rem' }}>
        {[
          ...
          <Link key={link.href} href={link.href} target={link.href === '/' ? '_blank' : undefined}
            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 1rem',
              background: '#1a1d27', border: '1px solid #2d3147', borderRadius: '8px',
